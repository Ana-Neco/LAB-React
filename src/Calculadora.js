...
function Calculator() {
  ...
  const clicou = (e) => {
    ...
    if (result === '0') {
      ...
      //Se result for '0', ele é substituído pelo valor do botão clicado. Caso contrário, o valor do botão é adicionado ao valor existente.
      } else {
  ...
  const porcentagem = () => {
    const numero = parseFloat(result);
    setResult((numero / 100).toString());
  };
  ...
  //A função eval é usada para avaliar uma string como código JavaScript.
  // Isso significa que ela interpreta o conteúdo da string como código JavaScript e o executa.
